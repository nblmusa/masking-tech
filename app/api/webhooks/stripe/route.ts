...
export async function POST(req: Request) {
	...
	if (event.type === "checkout.session.completed") {
		...
		if (session.metadata?.type === "bundle") {
			...
			if (userId && creditsToAdd > 0) {
				await incrementUserCredits(supabase, userId, creditsToAdd);
			}
		}
	}
	...
	if (event.type === "invoice.paid") {
		...
		if (subscription) {
			await incrementUserCredits(supabase, subscription.user_id, subscription.credits_per_month);
		}
	}
...
async function incrementUserCredits(supabase: any, userId: string, credits: number) {
